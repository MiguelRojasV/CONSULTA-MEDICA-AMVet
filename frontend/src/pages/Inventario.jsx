...
export default function Inventario() {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true)
    try {
      if (editing) { await api.put(`/inventario/${editing.id}`, form); toast.success('Actualizado') }
      else { await api.post('/inventario/', form); toast.success('Producto registrado') }
      setModal(false); load()
  ...
  const handleMovimiento = async (e) => {
    e.preventDefault()
    if (!movForm.cantidad || parseInt(movForm.cantidad) <= 0) { toast.error('Cantidad inválida'); return }
    try {
      await api.post(`/inventario/${movModal.id}/movimiento`, movForm)
