...
export default function Citas() {
  ...
  const handleSave = async (e) => {
    ...
    if (!form.paciente_id || !form.fecha || !form.hora) {
      toast.error('Paciente, fecha y hora son obligatorios'); return
    }
    setSaving(true)
    try {
      if (editing) {
        await api.put(`/citas/${editing.id}`, form)
  ...
  const filtered = citas.filter(c => {
    const matchSearch = (c.paciente_nombre || '').toLowerCase().includes(search.toLowerCase()) ||
