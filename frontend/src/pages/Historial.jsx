...
export default function Historial() {
  ...
  const handleSave = async (e) => {
    e.preventDefault()
    if (!form.paciente_id || !form.fecha) { toast.error('Paciente y fecha son obligatorios'); return }
    setSaving(true)
    try {
      if (editing) {
        await api.put(`/historial/${editing.id}`, form)
    ...
    <div>
      ...
      {detail && (
        <div className="modal-overlay" onClick={() => setDetail(null)}>
          <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">📋 Registro Clínico — {detail.paciente_nombre}</h3>
