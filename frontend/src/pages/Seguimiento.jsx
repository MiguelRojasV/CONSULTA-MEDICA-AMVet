...
export function Seguimiento() {
  ...
  const handleSave = async (e) => {
    e.preventDefault()
    if (!form.paciente_id || !form.fecha_seguimiento) { toast.error('Paciente y fecha son obligatorios'); return }
    setSaving(true)
    try {
      if (editing) { await api.put(`/seguimiento/${editing.id}`, form); toast.success('Actualizado') }
      else { await api.post('/seguimiento/', form); toast.success('Seguimiento registrado') }
