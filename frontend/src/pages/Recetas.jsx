...
export function Recetas() {
  ...
  const handleSave = async (e) => {
    e.preventDefault()
    if (!form.paciente_id || form.items.length === 0) { toast.error('Paciente y medicamentos son obligatorios'); return }
    setSaving(true)
    try {
              ...
              {recetas.length === 0
                ...
                : recetas.map((r, i) => (
                  <tr key={r.id}>
                    ...
                    <td>{r.fecha}</td>
                    <td className="font-medium">{r.paciente_nombre}</td>
            ...
            <div className="modal-body">
              <div style={{ border: '2px solid var(--border)', borderRadius: 10, padding: 20 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, fontSize: '0.875rem' }}>
                  <div><strong>Paciente:</strong> {detail.paciente_nombre}</div>
